...
type PromoType = "total" | "perUnit" | "none";
...
const POSTS: Record<string, {
  ...
  "1": { sector: "강남구", round: 1, date: "2026년 6월 15일 (일)", deadline: "18:00", hub: "성수 유닛", volume: 45, basePrice: 2500, promoType: "total",   promoPrice: 3000, total: 5, filled: 2, postType: "긴급"   },
  "2": { sector: "서초구", round: 1, date: "2026년 6월 15일 (일)", deadline: "18:00", hub: "성수 유닛", volume: 38, basePrice: 2500, promoType: "none",    promoPrice: 0,    total: 3, filled: 1, postType: "모집중"  },
  "3": { sector: "송파구", round: 2, date: "2026년 6월 15일 (일)", deadline: "21:30", hub: "성수 유닛", volume: 52, basePrice: 2800, promoType: "perUnit", promoPrice: 500,  total: 4, filled: 3, postType: "수기공고" },
  "4": { sector: "강동구", round: 1, date: "2026년 6월 16일 (월)", deadline: "18:00", hub: "성수 유닛", volume: 40, basePrice: 2500, promoType: "none",    promoPrice: 0,    total: 4, filled: 1, postType: "모집중"  },
  "5": { sector: "마포구", round: 2, date: "2026년 6월 16일 (월)", deadline: "21:30", hub: "마포 유닛", volume: 48, basePrice: 2600, promoType: "perUnit", promoPrice: 500,  total: 3, filled: 1, postType: "긴급"   },
  "6": { sector: "부천시", round: 1, date: "2026년 6월 17일 (화)", deadline: "18:00", hub: "부천 유닛", volume: 35, basePrice: 2400, promoType: "none",    promoPrice: 0,    total: 3, filled: 1, postType: "수기공고" },
  "7": { sector: "강서구", round: 1, date: "2026년 6월 15일 (월)", deadline: "18:00", hub: "강서 유닛", volume: 42, basePrice: 2600, promoType: "perUnit", promoPrice: 300,  total: 3, filled: 2, postType: "긴급"   },
};
...
const DELIVERY_ZONES: Record<string, { area: string; x: number; y: number }[]> = {
  ...
  "4": [
    { area: "강동구 천호동", x: 32, y: 38 },
    { area: "강동구 성내동", x: 58, y: 25 },
    { area: "강동구 암사동", x: 75, y: 55 },
  ],
  "5": [
    { area: "마포구 합정동", x: 28, y: 42 },
    { area: "마포구 망원동", x: 50, y: 25 },
    { area: "마포구 상암동", x: 72, y: 35 },
    { area: "마포구 성산동", x: 60, y: 62 },
  ],
  "7": [
    { area: "강서구 화곡동", x: 25, y: 35 },
    { area: "강서구 가양동", x: 55, y: 25 },
    { area: "강서구 등촌동", x: 72, y: 55 },
    { area: "강서구 마곡동", x: 40, y: 65 },
  ],
};
...
function UrgentDetailContent() {
  ...
  const isDemoFull  = params.get("demo") === "full";
  ...
  const handleApply = async () => {
    ...
    if (remain <= 0 || isDemoFull) { setShowFull(true); return; }
    setApplying(true);
    const rank = await fetchUserRank(isZRankDemo);
    setApplying(false);
    if (rank === "Z") { setShowContactOps(true); return; }
  ...
  const isManual = post.postType === "수기공고";
    ...
    <div className="flex justify-center items-start min-h-screen bg-[#111]">
          ...
          <div className="px-5 pt-5 pb-4">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-[12px] font-semibold px-2.5 py-1 rounded-full"
                style={
                  post.postType === "긴급"
                    ? { backgroundColor: "#FFF3EC", color: "#FF6B00" }
                    : post.postType === "수기공고"
                    ? { backgroundColor: "#EAF4FF", color: "#007AFF" }
                    : { backgroundColor: "#E8F9ED", color: "#34C759" }
                }>
                {post.postType}
              </span>
            </div>
              ...
              <div className="rounded-2xl overflow-hidden border border-[#E5E5EA]">
                <div className="relative bg-[#EBF0F6] h-[200px]">
                  <div className="absolute inset-0 opacity-40"
                    style={{
                      backgroundImage: "linear-gradient(#B8C8D8 1px, transparent 1px), linear-gradient(90deg, #B8C8D8 1px, transparent 1px)",
            ...
            {post.promoType === "total" ? (
              ...
              <div className="bg-surface rounded-2xl overflow-hidden">
                ...
                <div className="px-4 py-3 flex justify-between items-center">
                  <span className="text-[13px] font-medium text-primary">예상 총 보상</span>
                  <div className="flex flex-col items-end gap-0.5">
                    <span className="text-[15px] font-bold text-primary">약 {baseEarning.toLocaleString()}원</span>
                    <span className="text-[11px] text-text-caption">{post.basePrice.toLocaleString()}원 × {post.volume}건</span>
                  </div>
                </div>
              </div>
            )}
          ...
          <div className="px-5 pt-5">
            ...
            {showPenalty && (
              <div className="bg-[#FFF8E7] rounded-2xl p-4 mt-2 space-y-2">
                {[
                  "배차 확정 후 취소 시 페널티가 부과될 수 있어요",
                  "배차 확정 즉시 Push알림이 발송돼요",
      ...
      {showConfirm && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-end justify-center"
          ...
          <div className="w-[375px] bg-white rounded-t-3xl px-5 pt-6 pb-2"
            ...
            <p className="text-[13px] text-[#8E8E93] text-center mb-5">아래 내용을 확인해주세요</p>
        ...
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center px-6">
          <div className="w-full max-w-[323px] bg-white rounded-3xl px-6 py-8 flex flex-col items-center text-center shadow-xl">
            ...
            <p className="text-[14px] text-[#8E8E93] leading-relaxed mb-7">
              현재 자동 배차가 어렵습니다.<br/>
              <span className="text-[#FF9500] font-semibold">1:1 문의하기로 연락해주세요.</span>
            </p>
            <button onClick={() => setShowContactOps(false)}
              className="w-full h-[52px] rounded-[14px] bg-[#6262EE] text-white text-[17px] font-semibold active:opacity-80">
              확인
            </button>
          </div>
      ...
      {showFull && (
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center px-6">
          <div className="w-full max-w-[323px] bg-white rounded-3xl px-6 py-8 flex flex-col items-center text-center shadow-xl">
            ...
            <h3 className="text-[20px] font-bold text-[#1C1C1E] mb-2">이미 배차가 완료되었습니다</h3>
            <p className="text-[14px] text-[#8E8E93] leading-relaxed mb-7">
              신청하는 사이 자리가 마감됐어요.<br/>다른 공고를 확인해보세요.
            </p>
            <button onClick={() => { setShowFull(false); router.push("/urgent-list"); }}
              className="w-full h-[52px] rounded-[14px] bg-[#6262EE] text-white text-[17px] font-semibold active:opacity-80">
              다른 공고 보기
            </button>
          </div>
      ...
      {showSuccess && (
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center px-6">
          <div className="w-full max-w-[323px] bg-white rounded-3xl px-6 py-8 flex flex-col items-center text-center shadow-xl">
            ...
            <p className="text-[14px] text-[#8E8E93] leading-relaxed mb-7">
              {post.sector} 지역 추가 배차가 확정되었습니다.<br/>Push알림으로 결과를 확인할 수 있어요.
            </p>
            <button onClick={() => router.push("/dispatch-list")}
              className="w-full h-[52px] rounded-[14px] bg-[#6262EE] text-white text-[17px] font-semibold active:opacity-80">
              신청 내역 확인
            </button>
          </div>
